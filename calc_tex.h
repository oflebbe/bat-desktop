...
#pragma omp parallel for
        for (unsigned int w = 0; w < TEXTURE_WIDTH; w++)
        {
            ...
            {
                flo_pixmap_set_pixel(p, w, h, map_hsv_rotated(hsv, 1.0f - flo_matrix_get_value(mat, w, h)));
            }
        }
    /*    if (tex)
        {
...
#pragma omp parallel for
        for (unsigned int w = 0; w < TEXTURE_WIDTH; w++)
        {
            for (unsigned int h = 0; h < height; h++)
            {
                unsigned real_w = w + h * TEXTURE_WIDTH;
                flo_pixmap_set_pixel(p, w, h, flo_hsvToRgb565(1.0f - flo_matrix_get_value(mat, real_w, h), 0.8, 0.8));
            }
        }
        if (tex)
        {
